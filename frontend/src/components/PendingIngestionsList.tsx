  ...
  const handleQuickConfirm = (ingestion: PendingIngestion) => {
    ...
    confirmMutation.mutate({
      ...
      userConfirmed: {
        ...
        notes: ingestion.ai_parsed.notes || ingestion.raw_msg
      }
    }, {
      onSettled: () => {
      ...
      <div className="flex flex-col gap-3">
        {ingestions.map((ingestion) => {
          ...
          return (
            <div
              key={ingestion.id}
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex flex-col gap-3 shadow-sm hover:shadow transition-shadow relative overflow-hidden"
            >
                ...
                {ingestion.ai_parsed.suggested_account_creation && ingestion.ai_parsed.suggested_account_creation.length > 0 && (
                  <div className="flex flex-col gap-2 col-span-2">
                    {ingestion.ai_parsed.suggested_account_creation.map((suggestion, idx) => {
                      const isEditing = editingSuggestion?.ingestionId === ingestion.id && editingSuggestion?.idx === idx
                      return (
                        ...
                        <div className="flex justify-between items-start">
                          ...
                          <div className="flex flex-col gap-1 shrink-0">
                            {isEditing ? (
                              <>
                                <button
                                  onClick={() => handleCreateSuggestedAccount(editingSuggestion.data, ingestion.id)}
                                  disabled={processingIds.includes(ingestion.id) || !editingSuggestion.data.name || !editingSuggestion.data.account_group}
                              ...
                              <>
                                <button
                                  onClick={() => handleCreateSuggestedAccount(suggestion as any, ingestion.id)}
                                ...
                                <button
                                  onClick={() => setEditingSuggestion({ ingestionId: ingestion.id, idx, data: { name: suggestion.name, account_group: suggestion.account_group, type: suggestion.type, description: (suggestion as any).description || '' } })}
                                  disabled={processingIds.includes(ingestion.id)}
              ...
              <div className="flex gap-2 justify-end border-t border-slate-100 dark:border-slate-800/80 pt-3">
                ...
                <button
                  onClick={() => handleQuickConfirm(ingestion)}
                  disabled={processingIds.includes(ingestion.id)}
